...
# Métricas
# ------------------------------------------------------------
# (la métrica operativa de la tesis, Captura@k sobre conteos, se define
#  dentro del pipeline como captura_at_k)
...
with st.sidebar:
    ...
    # Modelos de CONTEO de la tesis final (V4.1): misma verosimilitud Poisson
    # y mismos hiperparámetros del estudio.
    modelo_sel = st.selectbox("Modelo (tesis V4.1)",
                              ["XGBoost Poisson (referencia de la tesis)",
                               "HistGB Poisson", "GLM Poisson (interpretable)"])
    frac_corte = st.slider("Corte temporal (fracción del periodo para entrenar)",
                           0.3, 0.8, 0.6, 0.05,
                           help="Validación temporal como en la tesis: se entrena con el "
                                "periodo inicial y se evalúa el ranking contra el resto.")
...
def mapa_prioridad(inter, aristas_ll):
    ...
    import folium
    p_nodo = inter["pct_p"].to_dict()                 # percentil del EB (tesis)
    top_resid = set(inter[inter["y"] == 1].nsmallest(12, "p_sorpresa").index)
    top_prev  = set(inter[inter["y"] == 0].nlargest(12, "lambda_oof").index)
    tr = aristas_ll.reset_index()
    def p_de(u, v):
...
if ejecutar:
    ...
    with st.spinner("Geocodificando, construyendo características y entrenando el modelo…"):
        ...
        val = loc[loc["inter_id"] != -1]
        agg = (val.groupby("inter_id")
               .agg(n_acc=("codigo", "count"), severidad=("severidad", "sum"),
                    muertes=("muertes", "sum"), heridos=("heridos", "sum")))
        # Conteos por PERIODO para la validación temporal (como en la tesis)
        f_ok = val["fecha"].notna()
        if f_ok.any():
            f0, f1 = val.loc[f_ok, "fecha"].min(), val.loc[f_ok, "fecha"].max()
            corte_fecha = f0 + (f1 - f0) * frac_corte
            agg["n_pre"] = val[val["fecha"] <= corte_fecha].groupby("inter_id").size()
        else:
            agg["n_pre"] = agg["n_acc"]
        inter = inter.join(agg).fillna({"n_acc": 0, "severidad": 0, "muertes": 0,
                                        "heridos": 0, "n_pre": 0})
        inter["n_post"] = inter["n_acc"] - inter["n_pre"]
        inter["y"] = (inter["n_acc"] > 0).astype(int)   # binaria SOLO para métricas/mapas
        ...
        # ============================================================
        # MODELACIÓN — EXACTA a la tesis final (V4.1):
        #  * objetivo = CONTEO n_acc (SPF), no binaria
        #  * features depuradas: sin dens_* individuales (generadores_300m
        #    es su suma exacta -> colinealidad perfecta) y distancias log1p
        #  * CV espacial GroupKFold -> lambda out-of-fold
        #  * métricas: Spearman, Captura@10%, ROC-AUC(>=1)
        #  * validación temporal + EMPIRICO-BAYES vs baselines
        #  * p_sorpresa = P(N>=n | lambda, alpha) para inspección
        # ============================================================
        FEAT = (["grado", "betweenness", "generadores_300m", "dist_centro"]
                + [f"dist_{c}" for c in CATEGORIAS_POI])
        X = inter[FEAT].fillna(inter[FEAT].median()).copy()
        LOGC = ["dist_centro"] + [f"dist_{c}" for c in CATEGORIAS_POI]
        X[LOGC] = np.log1p(X[LOGC])                      # log1p como en la tesis
        y_count = inter["n_acc"].values.astype(float)
        y = inter["y"].values                             # binaria auxiliar
        grupos = inter["bloque"].values

        def crea_modelo():
            # Hiperparámetros EXACTOS de la tesis (tabla de configuración)
            if modelo_sel.startswith("GLM"):
                return make_pipeline(StandardScaler(),
                                     PoissonRegressor(alpha=1.0, max_iter=2000))
            if modelo_sel.startswith("HistGB"):
                return HistGradientBoostingRegressor(loss="poisson", max_iter=300,
                                                     max_depth=3, learning_rate=0.05,
                                                     random_state=42)
            from xgboost import XGBRegressor
            return XGBRegressor(objective="count:poisson", n_estimators=300,
                                max_depth=3, learning_rate=0.05, subsample=0.8,
                                colsample_bytree=0.8, random_state=42)

        def captura_at_k(n_true, score, k=0.10):
            # fracción de TODOS los siniestros en el top k% del ranking
            nk = max(1, int(len(score) * k))
            return n_true[np.argsort(score)[-nk:]].sum() / max(1.0, n_true.sum())

        if y.sum() < n_folds or (y == 0).sum() < n_folds:
            avisos.append(f"Pocos cruces con siniestros ({int(y.sum())}) para "
                          f"{n_folds} pliegues; la validación puede ser inestable.")

        oof = np.zeros(len(y_count))
        n_ok = min(n_folds, pd.Series(grupos).nunique())
        try:
            for tr, te in GroupKFold(n_splits=n_ok).split(X, y_count, grupos):
                m = crea_modelo(); m.fit(X.iloc[tr], y_count[tr])
                oof[te] = np.clip(m.predict(X.iloc[te]), 0, None)
            metrico = True
        except Exception as e:
            avisos.append(f"No se pudo completar la CV espacial ({e}); ajuste "
                          f"completo sin métricas OOF.")
            m = crea_modelo(); m.fit(X, y_count)
            oof = np.clip(m.predict(X), 0, None); metrico = False

        inter["lambda_oof"] = oof

        # ----- VALIDACIÓN TEMPORAL + EMPÍRICO-BAYES (bloque 13B de la tesis) -----
        n_pre = inter["n_pre"].values.astype(float)
        n_post = inter["n_post"].values.astype(float)
        alpha_mm = max((n_pre.var() - n_pre.mean()) / max(n_pre.mean(), 1e-9) ** 2, 1e-6)
        res_t = None
        if n_post.sum() >= 5:
            m_t = crea_modelo(); m_t.fit(X, n_pre)
            lam_t = np.clip(m_t.predict(X), 1e-6, None)
            w_eb_t = 1.0 / (1.0 + alpha_mm * lam_t)
            def fila_t(nombre, s):
                return {"ranking": nombre,
                        "Spearman": spearmanr(n_post, s)[0],
                        "Captura@10%": captura_at_k(n_post, s),
                        "ROC-AUC (>=1)": (roc_auc_score((n_post > 0).astype(int), s)
                                          if 0 < (n_post > 0).sum() < len(n_post) else np.nan)}
            res_t = pd.DataFrame([
                fila_t("EB: modelo + historia", w_eb_t * lam_t + (1 - w_eb_t) * n_pre),
                fila_t("Modelo (pre→post)", lam_t),
                fila_t("BASELINE historia (n_pre)", n_pre),
                fila_t("BASELINE nulo (-dist_centro)", -inter["dist_centro"].values),
            ]).set_index("ranking")
        else:
            avisos.append("Muy pocos siniestros en el periodo de evaluación para la "
                          "validación temporal; ajusta el corte temporal.")

        # ----- EB del periodo completo: el RANKING OPERATIVO (como en la tesis) -----
        w_eb = 1.0 / (1.0 + alpha_mm * oof)
        inter["eb"] = w_eb * oof + (1 - w_eb) * y_count
        inter["pct_p"] = inter["eb"].rank(pct=True)        # percentil EB -> mapas

        # ----- p_sorpresa bajo binomial negativa (inspección de campo) -----
        r_nb = 1.0 / alpha_mm
        p_nb = r_nb / (r_nb + np.clip(oof, 1e-6, None))
        inter["p_sorpresa"] = nbinom.sf(y_count - 1, r_nb, p_nb)
        ...
        st.session_state["R"] = {
            ...
            "metrico": metrico, "metr": metr, "avisos": avisos, "res_t": res_t,
            "alpha_mm": float(alpha_mm),
...
with t3:
    if R["metrico"]:
        ...
        figcap = px.line(x=M["curva_x"], y=M["curva_y"],
                         ...
                         title=f"Curva de captura (OOF espacial · @10% = {100*M['cap10']:.1f}%)",
                         color_discrete_sequence=["#d7301f"])
        figcap.add_shape(type="line", line=dict(dash="dash", color="gray"),
                         x0=0, y0=0, x1=1, y1=1)
        figcap.add_vline(x=0.10, line_dash="dot", line_color="gray")
    ...
    if R.get("res_t") is not None:
        st.dataframe(R["res_t"].round(3), use_container_width=True)
        st.caption(f"Entrenado con el periodo inicial y evaluado contra el resto; "
                   f"EB = w·λ + (1−w)·historial con w = 1/(1+α·λ), α = {R['alpha_mm']:.2f} "
                   f"(método de momentos). Como en la tesis, el EB debe superar al modelo "
                   f"solo y a la historia sola.")
    else:
        st.info("Sin suficientes siniestros en el periodo de evaluación (ajusta el corte temporal).")

    st.subheader("Más peligrosas de lo esperado (inspección de campo, menor p_sorpresa)")
    tt = inter.join(R["ref"])
    st.dataframe(tt[tt["y"] == 1].nsmallest(10, "p_sorpresa")
                 [["direccion", "n_acc", "severidad", "lambda_oof", "p_sorpresa"]].round(4))
    st.subheader("Riesgo estructural sin siniestros (prevención, mayor λ)")
    st.dataframe(inter[inter["y"] == 0].nlargest(10, "lambda_oof")
                 [["n_acc", "lambda_oof", "grado", "betweenness", "generadores_300m"]].round(3))
